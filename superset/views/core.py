...
class Superset(BaseSupersetView):
    ...
    @expose("/iframe/<token>/<dashboard_id>", methods=["GET"])
    def iframe_hf(self, token, dashboard_id) -> Response:
        from superset.datacube import utils
        import flask_login.utils as flask_lg
        user_name, user_passwd = utils.get_user_info_by_token(token)
        if user_name == None:
            return "user_name is None"
        user = self.appbuilder.sm.auth_user_db(user_name, user_passwd)
        if user == None:
            return "user is  not exits"
        print("token:" + token, "username:" + user_name, "dashboard_id:" + dashboard_id)
        flask_lg.login_user(user)
        print(user_name,user_passwd)
        print(user)
        from superset.models.user_dashboard_rel import UserDashboardRel
        condition={}
        condition["user_id"]=user.id
        condition["dashboard_id"] = dashboard_id
        lists=UserDashboardRel.get_data_by_multiple_condition(condition)
        print(lists)
        if len(lists) == 0 :
            return "n"
        # self.assertNotIn("User confirmation needed", resp)
        return self.dashboard_all(dashboard_id,True)

    @has_access
    @expose("/iframe_hf/<dashboard_id>/")
    def dashboard_hf(self, dashboard_id):
        return self.dashboard_all(dashboard_id,True)
    ...
    def dashboard(self, dashboard_id):
        return self.dashboard_all(dashboard_id, False)

    def dashboard_all(self, dashboard_id, iframe):
        resp_html = "superset/dashboard.html"
        if iframe:
            resp_html = "superset/dashboard_hf.html"
